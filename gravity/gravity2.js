const c = document.getElementById("gameCanvas");
        ...
        //funzione disegno pianeti 
        
        function drawPlanets(){
            for (let i = 0; i < currentLevel.numPlanets; i++) {

                if((currentLevel.planets[i].x + currentLevel.planets[i].r) - player.x < 700 || (currentLevel.planets[i].y + currentLevel.planets[i].r) < 400){
                ctx.beginPath();
                ctx.arc(currentLevel.planets[i].x - player.x + 700, currentLevel.planets[i].y - player.y + 400, currentLevel.planets[i].r, 0, Math.PI * 2);
                ctx.fillStyle = "blue";
                ctx.fill();
                ctx.closePath();
                }
